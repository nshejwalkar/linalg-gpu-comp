...
# v5 = v4 (sync-free + CUDA graphs) with the CPU-scalar -> CUDA assignments removed
# so graph capture actually succeeds:
#   - `Y[:, idx, idx] = 1.0`      -> tril(.,-1) + device eye      (no CPU scalar copy)
#   - `tau_all[:, col] = 0.0`     -> in-place .zero_()            (no CPU scalar copy)
...
def _build_wy(H, tau_all, k, b, n):
    ...
    dtype = H.dtype
    panel = H[:, k:, k:k + b]
    Y = torch.tril(panel, diagonal=-1)              # strictly-lower (diag = 0)
    eye_b = torch.eye(b, device=device, dtype=dtype)
    Y[:, :b, :] = Y[:, :b, :] + eye_b               # device-side unit diagonal
