...
# CUDA-graphed blocked-WY Householder QR + shape dispatch.
# Graph replay removes the CPU-dispatch overhead that dominates this launch-bound
# workload; dispatch sends shapes where cuSOLVER wins (tiny n, small-batch/large-n)
# to torch.geqrf. Measured win regime on B200: 128 <= n <= 1024.
...
def _householder_step(H, tau_all, col, col_end):
    ...
    m = x.shape[-1]
    if m <= 1:
        tau_all[:, col].zero_()                       # device-side (graph-safe)
        return
    alpha = x[:, 0]
    norm_x = x.norm(dim=-1)
    sign_a = alpha.sign()
    sign_a = torch.where(sign_a == 0, torch.ones_like(sign_a), sign_a)  # no nonzero sync
    beta = -sign_a * norm_x
    v0 = alpha - beta
...
def _build_wy(H, tau_all, k, b, n):
    ...
    Y[:, :b, :] = Y[:, :b, :] + eye_b                 # device-side unit diagonal (graph-safe)
...
def _graphed_qr(data):
    ...
    if entry is _MISS:
        try:
            ...
            entry = None  # capture unsupported → eager fallback (cached, no retry)
...
def _ours_wins(batch: int, n: int) -> bool:
    # Measured graphed-vs-geqrf crossover on B200: we win 128<=n<=1024
    # (n176 1.77x, n352 2.01x, n512 16.5x, n1024 2.75x). Lose n32 (overhead) and
    # n>=2048 small-batch (cuSOLVER single-matrix path wins).
    return 128 <= n <= 1024
