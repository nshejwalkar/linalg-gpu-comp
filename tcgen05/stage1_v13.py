"""
stage1_v13.py — tcgen05 BF16x9 GEMM, v13.

FIX v12 ERROR:
  Expected input tensor to have the profile (MMA,MMA_MN,MMA_K,...) with at least 3 modes.
  Caused by make_fragment_A(sA) where sA is a flat 2D tensor.
  make_fragment_A expects a PARTITIONED tensor (output of ThrMma.partition_A).

ROOT CAUSE: we should not call make_fragment_A manually.
  cute.gemm(mma_atom, d, sA, sB, c) handles SMEM->descriptor conversion internally
  in the MLIR lowering.

KEY FINDINGS:
  1. cute.gemm docs: "CORRECT: MMA without elect_one" — no elect_one wrapper
  2. algorithm.py gemm: takes atom + tensors, passes .value directly to _cute_ir.gemm
  3. The AlgorithmGemmOp (cute.gemm MLIR op) handles SMEM tensors natively
  4. No need to call make_fragment_A or make_umma_smem_desc manually

REVISED APPROACH:
  - allocate sA, sB with swizzle in pointer (affine layout) -- required by _verify_fragment
  - call cute.gemm(mma_atom, tCtAcc, sA, sB, tCtAcc) directly, NO elect_one

  But wait: algorithm.py says ranks must match:
    a_rank == 2: c_rank must be 2 or 3
  sA is (TILE_M, BK) = rank 2, tCtAcc is (TILE_M, TILE_N) = rank 2 -> OK for dispatch [3]

  Dispatch [3]: (M,K) x (N,K) => (M,N)   [BUT B is (K,N) not (N,K)?]
  Actually cute.gemm just passes tensors.value to _cute_ir.gemm.
  The MLIR op knows how to dispatch based on MMA atom type.
...
app = modal.App("qr-tcgen05-stage1-v13")


_KERNEL_SRC = r'''"""
tcgen05 BF16->FP32 v13.
Fix: use cute.gemm(mma_atom, d, sA, sB, c) directly.
No elect_one, no make_fragment_A, no make_umma_smem_desc.
SMEM tensors have affine layout (swizzle in pointer).
"""
...
def _mma_kernel_clear(
    ...
    # MMA warp: NO elect_one (tcgen05 docs say DO NOT use elect_one with cute.gemm)
    if warp_id == 2:
        ...
        # cute.gemm handles SMEM tensor -> descriptor conversion internally
...
def smoke_test():
    ...
    print("=" * 72)
    print("STAGE 1 V13 SMOKE — cute.gemm direct, no elect_one, no make_fragment_A")
    print("=" * 72)

    kpath = "/root/_stage1_v13_kernel.py"
    with open(kpath, "w") as f:
        f.write(_KERNEL_SRC)

    spec = importlib.util.spec_from_file_location("_stage1_v13_kernel", kpath)
    kmod = importlib.util.module_from_spec(spec)
    sys.modules["_stage1_v13_kernel"] = kmod
    try:
...
def bf16x9_test():
    ...
    print("=" * 72)
    print("STAGE 1 V13 BF16x9 — Ozaki split, 9 passes, K=16")
    print("=" * 72)

    kpath = "/root/_stage1_v13_bf16x9.py"
    with open(kpath, "w") as f:
        f.write(_KERNEL_SRC)

    spec = importlib.util.spec_from_file_location("_stage1_v13_bf16x9", kpath)
    kmod = importlib.util.module_from_spec(spec)
    sys.modules["_stage1_v13_bf16x9"] = kmod
    try:
        spec.loader.exec_module(kmod)
    except Exception as e:
        print(f"  LOAD FAILED: {type(e).__name__}: {e}")
...
def main(stage: str = "smoke"):
    print(f">>> stage1_v13 stage={stage}")
    if stage == "smoke":
