"""
opus_probe_mma.py — isolate misaligned address into MMA-path vs TMEM-epilogue-path.
Two LAUNCHED kernels (SWZ load proven OK already):
  K_mma:  load + alloc_tmem + cute.gemm + commit + mbarrier_wait; write dummy 1.0 to gC.
          (no TMEM read) -> isolates MMA issue / commit / tmem alloc.
  K_epi:  alloc_tmem + make_tmem_copy + TMEM->reg->GMEM (TMEM uninit) -> isolates epilogue.
...
app = modal.App("qr-opus-probe-mma")
...
@cute.kernel
def _k_mma(gA: cute.Tensor, gB: cute.Tensor, gC: cute.Tensor):
    ...
    arch.sync_threads()
    # dummy write, NO tmem read
    if warp_id==0:
        for i in range(TILE_M*TILE_N//32):
            idx=lane_id+i*32; m=idx//TILE_N; n=idx%TILE_N; gC[m,n]=Float32(1.0)
    arch.sync_threads()
    if warp_id==0 and lane_id==0: arch.dealloc_tmem(tmem_ptr, TILE_N)

@cute.kernel
def _k_epi(gA: cute.Tensor, gB: cute.Tensor, gC: cute.Tensor):
    warp_id = arch.warp_idx(); lane_id = arch.lane_idx(); tidx,_,_ = arch.thread_idx()
    smem = SmemAllocator()
    smp = smem.allocate(Uint32)
    if warp_id==0 and lane_id==0:
...
def run_mma(gA: cute.Tensor, gB: cute.Tensor, gC: cute.Tensor):
    _k_mma(gA,gB,gC).launch(grid=(1,1,1), block=(128,1,1))
@cute.jit
def run_epi(gA: cute.Tensor, gB: cute.Tensor, gC: cute.Tensor):
    _k_epi(gA,gB,gC).launch(grid=(1,1,1), block=(128,1,1))
'''
...
def probe():
    ...
    with open("/root/_opus_probe_mma.py","w") as f:
        f.write(_K)
    spec = importlib.util.spec_from_file_location("_opus_probe_mma","/root/_opus_probe_mma.py")
    kmod = importlib.util.module_from_spec(spec); sys.modules["_opus_probe_mma"]=kmod
    spec.loader.exec_module(kmod)
    M,N,K = 128,256,16
    torch.manual_seed(0)
    A = torch.randn(M,K,device="cuda",dtype=torch.bfloat16)
    B = torch.randn(K,N,device="cuda",dtype=torch.bfloat16)
    for name, fn in [("MMA-only(dummy write)", kmod.run_mma), ("EPI-only(tmem copy)", kmod.run_epi)]:
        ...
        try:
            ...
            print(f"  LAUNCH OK. gC[0,:4]={[round(v,2) for v in C[0,:4].tolist()]}")
